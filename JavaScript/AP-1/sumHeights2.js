...
const sumHeights2 = (heights, start, end) => {
    let sum = 0;

    for (let i = start; i < end; i++) {
        if (heights[i] < heights[i + 1]) {
            sum = sum + (Math.abs(heights[i] - heights[i + 1]) * 2);
        } else {
...
console.log(sumHeights2([5, 3, 6, 7, 2], 2, 4));
console.log(sumHeights2([5, 3, 6, 7, 2], 0, 1));
console.log(sumHeights2([5, 3, 6, 7, 2], 0, 4));
